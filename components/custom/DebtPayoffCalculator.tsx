    ...
    <div className="not-prose my-8 p-5 sm:p-6 bg-gray-50 dark:bg-gray-900/60 border border-gray-200 dark:border-gray-700 rounded-2xl">
      ...
      <div className="pt-5 border-t border-dashed border-gray-200 dark:border-gray-700">
        <p className="font-serif text-lg sm:text-xl leading-snug mb-4 text-gray-900 dark:text-white">
          {base.months === null ? (
            <>At {money(payment)}/mo, you&apos;re barely covering interest — this balance <strong className="text-red-600 dark:text-red-400 dark:text-red-400">never gets paid off</strong>. Raise the payment.</>
          ) : (
            <>
              At {money(payment)}/mo, you&apos;re debt-free in{' '}
              <strong>{base.months} months</strong> and pay about{' '}
              <strong className="text-red-600 dark:text-red-400 dark:text-red-400">{money(base.totalInterest)}</strong> in interest.
              {saved > 0 && (
                <> Adding $50/mo saves about <strong className="text-emerald-700 dark:text-emerald-400">{money(saved)}</strong>.</>
              )}
            </>
          )}
        </p>
        ...
        {scenarios.map(s => (
          <div key={s.key} className="mb-4">
            <div className="flex justify-between font-mono text-[12px] mb-1.5">
              ...
              <span className={`font-medium ${Number.isFinite(s.totalInterest) && s.totalInterest === Math.min(...finiteInterests) ? 'text-emerald-700 dark:text-emerald-400' : 'text-gray-900 dark:text-white'}`}>
